#!/usr/bin/env python
"""TalkNet-ASD backend runner. Same as LR-ASD but talkNetModel adds a
forward_cross_attention step before the audio-visual backend. Runs in the
TalkNet-ASD venv with the repo on PYTHONPATH and cwd = repo root."""
    ...
    for duration in (1, 1, 1, 2, 2, 2, 3, 3, 4, 5, 6):
        ...
        with torch.no_grad():
            for i in range(batch):
                ...
                ea, ev = model.forward_cross_attention(ea, ev)
...
if __name__ == "__main__":
    _common.run(load_model, score_track, default_weight="pretrain_TalkSet.model")
